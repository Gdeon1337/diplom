defmodule DomoapiWeb.DeivceControllerTest do
    ...
    @create_attrs_house %{
      ...
      describe "index" do
        test "lists all devices", %{conn: conn} do
          ...
          conn = get(conn, Routes.tenant_device_path(conn, :index, tenant.id))
      ...
      describe "create device" do  
        test "renders device when data is valid", %{conn: conn} do
          ...
          conn = get(conn, Routes.tie_device_path(conn, :tie_the_device, binding_token: binding_token.id, device_type: "android", token: "token"))
          assert %{"device_type" => "android"} = json_response(conn, 201)
        end
      end
      ...
      describe "update device" do
        test "renders device when data is valid", %{conn: conn} do
          ...
          binding_token = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
          conn = get(conn, Routes.tie_device_path(conn, :tie_the_device, binding_token: binding_token.id, device_type: "android", token: "token"))
          device = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
          conn = put(conn, Routes.tenant_device_path(conn, :update, tenant.id, device.id), device: @update_attrs)
          conn = get(conn, Routes.tenant_device_path(conn, :show, tenant.id, device.id))
          assert %{"device_type" => "update title"} = json_response(conn, 200)
        end
      end
      ...
      describe "delete device" do
        test "deletes chosen device", %{conn: conn} do
            ...
            conn = get(conn, Routes.tie_device_path(conn, :tie_the_device, binding_token: binding_token.id, device_type: "android", token: "token"))
            device = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
            conn = delete(conn, Routes.tenant_device_path(conn, :delete, tenant.id, device.id))
