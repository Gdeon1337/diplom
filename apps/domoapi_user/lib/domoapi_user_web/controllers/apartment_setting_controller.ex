defmodule DomoapiUserWeb.ApartmentSettingContoller do
    ...
    def create(conn, apartment_setting_params) do
      tenant = GPlug.current_resource(conn)
      apartment_setting_params = Map.put(apartment_setting_params, "apartment_id", tenant.apartment_id)
      intercom = Intercoms.get_intercom(%{"tenant_id" => tenant.id})
      with {:ok, %ApartmentSetting{} = apartment_setting} <- Place.create_apartment_setting(apartment_setting_params),
      {:ok, pid} <- Task.start(fn -> Place.cast_setting_task(intercom.host_name, apartment_setting, intercom.serial_key, 3) end) do
        conn
        |> put_status(:created)
    ...
    swagger_path(:show) do
      get("/users/apartment_settings/{id}")
    ...
    swagger_path(:update) do
      security [%{Bearer: []}]
      put("/users/apartment_settings/{id}")
      parameter(:apartment_id, :path, :string, "apartment ID type uuid", required: true, example: "9b0670e2-fa51-4dca-8c5a-0a5c9fbda22f")
    ...
    def update(conn, %{"id" => id, "apartment_setting" => setting_params}) do
      tenant = GPlug.current_resource(conn)
      setting_params = Map.put(setting_params, "apartment_id", tenant.apartment_id)
      apartment_setting = Place.get_apartment_setting!(id)
      intercom = Intercoms.get_intercom(%{"tenant_id" => tenant.id})
      with {:ok, %ApartmentSetting{} = apartment_setting} <- Place.update_apartment_setting(apartment_setting, setting_params), 
      {:ok, pid} <- Task.start(fn -> Place.cast_setting_task(intercom.host_name, apartment_setting, intercom.serial_key, 3) end) do
        json(conn, apartment_setting)
      end
    end
    ...
    swagger_path(:delete) do
      PhoenixSwagger.Path.delete("/users/apartment_settings/{id}")
      security [%{Bearer: []}]
    ...
    def delete(conn, %{"id" => id}) do
      ...
      apartment_setting = Place.get_apartment_setting!(id)
      intercom = Intercoms.get_intercom(%{"tenant_id" => tenant.id})
      with {:ok, %ApartmentSetting{}} <- Place.delete_apartment_setting(apartment_setting),
      {:ok, pid} <- Task.start(fn -> Place.request_intercom_delete_room_setting(intercom.host_name, apartment_setting.apartment_id, intercom.serial_key, 3) end) do
        json(conn, %{})
      end
    end
  end
