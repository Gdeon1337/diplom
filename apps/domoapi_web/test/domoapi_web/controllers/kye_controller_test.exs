defmodule DomoapiWeb.KeyControllerTest do
    ...
    def fixture(:key) do
      {:ok, key} = Intercoms.create_key(@create_attrs)
      key
    end
    ...
    describe "index" do
      test "lists all keys", %{conn: conn} do
        ...
        conn = get(conn, Routes.intercom_key_path(conn, :index, intercom.id, page: 1, page_size: 10))
    ...
    describe "create key" do
      test "renders key when data is valid", %{conn: conn} do
        ...
        conn = post(conn, Routes.intercom_key_path(conn, :create, intercom.id), key)
        assert %{"id" => id} = json_response(conn, 201)
  
        conn = get(conn, Routes.intercom_key_path(conn, :show, intercom.id, id))
      ...
      test "renders errors when data is invalid", %{conn: conn} do
        ...
        conn = post(conn, Routes.intercom_path(conn, :create), intercom)
        intercom = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
        conn = post(conn, Routes.intercom_key_path(conn, :create, intercom.id), @invalid_attrs)
    ...
    describe "update key" do
      test "renders key when data is valid", %{conn: conn} do
        ...
        conn = post(conn, Routes.intercom_key_path(conn, :create, intercom.id), key)
        key = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
        conn = put(conn, Routes.intercom_key_path(conn, :update, intercom.id, key.id), key: @update_attrs)
        conn = get(conn, Routes.intercom_key_path(conn, :show, intercom.id, key.id))
        assert %{
                 "title" => "some updated title"
               } = json_response(conn, 200)
      end
    ...
    describe "delete key" do
      test "deletes chosen key", %{conn: conn} do
        conn = post(conn, Routes.house_path(conn, :create), @create_attrs_house)
        house =  for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
        intercom =
        %{ 
        ...
        conn = post(conn, Routes.intercom_key_path(conn, :create, intercom.id), key)
        key = for {key, val} <- json_response(conn, 201), into: %{}, do: {String.to_atom(key), val}
        conn = delete(conn, Routes.intercom_key_path(conn, :delete, intercom.id, key.id))
